...
#'Produces:
#'    
#'    - data/BIG_FILES/all_buffer/ -> one file per cell
...
occ_list <- get(load(
  file = here::here("data", "Loiseau2021", "birds_occurrences.RData")
))
Loiseau_et_al <- get(load(
  file = here::here("data", "Loiseau2021", "Loiseau_et_al.RData")
))
Loiseau_et_al <- Loiseau_et_al[Loiseau_et_al$class %in% "birds", ]
Loiseau_et_al <- Loiseau_et_al[, c("species_code", "scientific_name")]
Loiseau_et_al <- Loiseau_et_al[
  Loiseau_et_al$species_code %in% unique(unlist(occ_list)),
]
...
traits_for_hypervolumes[, 1:11] <- scale(
  log10(traits_for_hypervolumes[, 1:11]),
  center = F
)
# flightless birds have wing length == 0
traits_for_hypervolumes[
  traits_for_hypervolumes$Wing.Length < 0.1,
  c("Wing.Length", "Secondary1", "Tail.Length")
] <- 0

disttrait = funrar::compute_dist_matrix(
  traits_for_hypervolumes,
  metric = "euclidean"
)
...
pc_insured <- data.frame(pcoas$vectors[
  rownames(pcoas$vectors) %in% spe_insured,
])
pc_insurer <- data.frame(pcoas$vectors[
  rownames(pcoas$vectors) %in% spe_insurer,
])
...
fig_pc_insured_1 <- ggplot(pc_axes, aes(x = Axis1, y = Axis2)) +
  ...
  theme_bw() +
  geom_point(data = pc_insured, col = "black") +
  geom_point(data = pc_insured_Di_Q, col = "red")
...
fig_pc_insurer_1 <- ggplot(pc_axes, aes(x = Axis1, y = Axis2)) +
  ...
  xlim(min(pcoas$vectors[, 1]), max(pcoas$vectors[, 1])) +
  theme_bw() +
  geom_point(data = pc_insurer, col = "black") +
  geom_point(data = pc_insured_Di_Q, shape = 1, col = "red") +
  geom_circle(
...
fig_pc_insured_2 <- ggplot(pc_axes, aes(x = Axis1, y = Axis2)) +
  ...
  theme_bw() +
  geom_point(data = pc_insurer, col = "black") +
  geom_point(data = pc_insurer_Di_Q, col = "red")
...
fig_pc_insurer_2 <- ggplot(pc_axes, aes(x = Axis1, y = Axis2)) +
  ...
  theme_bw() +
  geom_point(data = pc_insured, col = "black") +
  geom_point(data = pc_insurer_Di_Q, shape = 1, col = "red") +
  geom_circle(
    aes(x0 = Axis1, y0 = Axis2, r = D_insu),
...
Insured_Insurer <- gridExtra::arrangeGrob(
  fig_pc_insured_1,
  fig_pc_insurer_1,
  fig_pc_insurer_2,
  fig_pc_insured_2,
...
ggsave(
  file = here::here("outputs", paste0(insured, "_vs_", insurer, ".tiff")),
  Insured_Insurer,
  width = 30,
  height = 30,
  dpi = 300,
      ...
      tryCatch(
        {
          ...
          tryCatch(
            {
              buffer_cells <- get(load(
                file = here::here(
                  "data",
                  'BIG_FILES',
                  "all_buffer",
                  paste0("cell_", focal_cell, ".RData")
                )
              ))
          ...
          set.seed(1)
          if (per_buff_all < 1)
            buffer_cells_used <- buffer_cells[
              which(
          ...
          buffer_cells_used <- buffer_cells_used[
            unlist(lapply(
              as.character(buffer_cells_used$neighbor),
              function(id) {
                length(unique(occ_list[[id]])) >= 5
              }
            )),
          ]
          ...
          if (
            ...
            I_buffer <- do.call(
              rbind,
              (parallel::mclapply(
                1:dim(buffer_cells_used)[1],
                function(j) {
                  #j=1

                  a <- Insu(
                    insured = focal_cell,
                    insurer = buffer_cells_used$neighbor[j],
                    occ_list,
                    dist_mat = disttrait,
                    D_insu,
                    D_thr
                  ...
                  b <- Insu(
                    insured = buffer_cells_used$neighbor[j],
                    insurer = focal_cell,
                    occ_list,
                    dist_mat = disttrait,
                    D_insu,
            ...
            buffer_com <- as.data.frame(matrix(
              0,
              length(birds_insurance_buffer),
              length(all_species)
            ))
            ...
            for (i in 1:length(birds_insurance_buffer)) {
              buffer_com[
                i,
                colnames(buffer_com) %in% occ_list[[rownames(buffer_com)[i]]]
              ] = 1
            }
...
ras <- suppressWarnings(raster::raster(here::here(
  "data",
  "Loiseau2021",
  "birds_richness.tif"
)))
...
low_threshold <- quantile(
  birds_insurance$Inward - birds_insurance$Outward,
  0.1,
  na.rm = TRUE
)
...
birds_insurance$SS_status <- ifelse(
  birds_insurance$Inward - birds_insurance$Outward >= high_threshold,
  "Sink",
  ifelse(
    birds_insurance$Inward - birds_insurance$Outward <= low_threshold,
    "Source",
    "Intermediate"
  )
)
...
FSA::dunnTest(
  birds_insurance$Nsp_loc ~ birds_insurance$SS_status,
  method = "bonferroni"
)
...
birds_insurance <- birds_insurance[
  order(birds_insurance$Nsp_loc, decreasing = F),
]
Fig_5c <- ggplot(
  birds_insurance,
  aes(x = Inward, y = Outward, color = Nsp_loc)
) +
  ...
  geom_abline(
    ...
    intercept = as.numeric(quantile(
      birds_insurance$Outward - birds_insurance$Inward,
      prob = 0.9,
      na.rm = T
    )),
    linetype = "dashed",
...
birds_insurance$SS = ((birds_insurance$Outward - birds_insurance$Inward) +
  abs(min(birds_insurance$Outward - birds_insurance$Inward, na.rm = T))) /
  200
birds_insurance$SS[
  (birds_insurance$Outward - birds_insurance$Inward >=
    quantile(
...
#sinks
ex <- birds_insurance[
  ((birds_insurance$Inward > 60) & (birds_insurance$Outward < 40)),
]
...
#sources
ex <- birds_insurance[
  ((birds_insurance$Inward < 40) & (birds_insurance$Outward > 60)),
]
ex <- ex[!is.na(ex$D_insu), ]
ex[ex$Nsp_loc > 120, ]
...
all_species <- unique(unlist(lapply(all_cells_buffer, function(id) {
  occ_list[[id]]
})))
all_species <- all_species[order(all_species)]
...
map_variable(
  ...
  disp_max = disp_max,
  reference = TRUE,
  grDevices::colorRampPalette(rev(RColorBrewer::brewer.pal(name = "RdBu", 9)))(
    255
  )
) #YlOrRd
map_variable(
  ...
  disp_max = disp_max,
  reference = TRUE,
  breaks = seq(0, 100, length.out = 256),
...
a <- ggplot(birds_insurance, aes(x = nsplog10, y = latitude, colour = Inward)) +
  ...
  scale_colour_gradientn(
    colours = rev(RColorBrewer::brewer.pal(n = 8, name = "YlGnBu"))
  ) +
  geom_smooth(
    aes(x = nsplog10),
    orientation = "y",
...
c <- ggplot(
  birds_insurance,
  aes(x = Outward, y = latitude, colour = nsplog10)
) +
...
terra::writeRaster(
  hi_reprojected,
  filename = here::here("data", "BIG_FILES", "hi_reprojected.tif"),
  filetype = "GTiff",
  overwrite = TRUE
)
...
Fig_6a <- ggplot(
  birds_insurance_hi,
  aes(x = SS_status, y = hi, fill = SS_status)
) +
...
FSA::dunnTest(
  birds_insurance_hi$hi ~ birds_insurance_hi$SS_status,
  method = "bonferroni"
)

save(
  birds_insurance_hi,
  file = here::here("results", "birds_insurance_hi.RData")
)
#----
...
shapefile_dirs <- c(
  here::here(
    "data",
    "BIG_FILES",
    "WDPA_Mar2025_Public_shp",
    "WDPA_Mar2025_Public_shp_0"
  ),
  here::here(
    "data",
    "BIG_FILES",
    "WDPA_Mar2025_Public_shp",
    "WDPA_Mar2025_Public_shp_1"
  ),
  here::here(
    "data",
    "BIG_FILES",
    "WDPA_Mar2025_Public_shp",
    "WDPA_Mar2025_Public_shp_2"
  )
)
...
wdpa_I_II <- wdpa_polygons_ter[
  wdpa_polygons_ter$IUCN_CAT %in% c("Ia", 'Ib', 'II'),
]
sf::sf_use_s2(FALSE)
wdpa_I_II_union <- sf::st_union(wdpa_I_II)
save(wdpa_I_II_union, file = here::here("results", "wdpa_I_II_union.RData"))

wdpa_I_IV <- wdpa_polygons_ter[
  wdpa_polygons_ter$IUCN_CAT %in% c("Ia", 'Ib', 'II', 'III', 'IV'),
]
sf::sf_use_s2(FALSE)
...
wdpa_extract_I <- exactextractr::exact_extract(
  grd,
  wdpa_I_union,
  include_area = TRUE
)
...
wdpa_extract_I_II <- exactextractr::exact_extract(
  grd,
  wdpa_I_II_union,
  include_area = TRUE
)
...
birds_insurance_hi_pa <- merge(
  birds_insurance_hi_pa,
  wdpa_extract_iucn_I_II,
  all = TRUE
)
...
wdpa_extract_I_IV <- exactextractr::exact_extract(
  grd,
  wdpa_I_IV_union,
  include_area = TRUE
)
...
birds_insurance_hi_pa <- birds_insurance_hi_pa[
  !is.na(birds_insurance_hi_pa$D_insu),
] #there is few D_insu NA (less than 400)

#save

birds_insurance_hi_pa$SS_status <- factor(
  birds_insurance_hi_pa$SS_status,
  levels = c("Source", "Intermediate", "Sink")
)
save(
  birds_insurance_hi_pa,
  file = here::here("results", "birds_insurance_hi_pa.RData")
)

#load
load(here::here("results", "birds_insurance_hi_pa.RData"))
birds_insurance_hi_pa <- birds_insurance_hi_pa[
  !is.na(birds_insurance_hi_pa$SS_status),
]
...
for (status in levels(birds_insurance_hi_pa$SS_status)) {
  ...
  subset_data <- birds_insurance_hi_pa[
    birds_insurance_hi_pa$SS_status == status,
  ]
...
summary_table$SS_status <- factor(
  summary_table$SS_status,
  levels = c("Source", "Intermediate", "Sink")
)
...
#for the cell with PA (>1%) plot the % of protection, Fig_6b

sub_birds_insurance_pa <- birds_insurance_hi_pa[
  birds_insurance_hi_pa$PA_I_IV >= ThtPA,
]

box_labels <- aggregate(
  PA_I_IV ~ SS_status,
  data = sub_birds_insurance_pa,
...
Fig_6b <- ggplot(
  ...
  aes(x = SS_status, y = PA_I_IV, fill = SS_status)
) +
  geom_boxplot(outlier.shape = NA) +
  # Add count labels
  # geom_text(data = box_labels,
...
ggsave(
  file = here::here("outputs", "Fig_6.tiff"),
  Fig_6,
  width = 20,
  height = 8,
  dpi = 300,
...
kruskal.test(
  PA_I_IV ~ SS_status,
  data = birds_insurance_hi_pa[birds_insurance_hi_pa$PA_I_IV >= 1, ]
)
FSA::dunnTest(
  birds_insurance_hi_pa[birds_insurance_hi_pa$PA_I_IV >= 1, ]$PA_I_IV ~
    birds_insurance_hi_pa[birds_insurance_hi_pa$PA_I_IV >= 1, ]$SS_status,
  method = "bonferroni"
)
...
kruskal.test(
  PA_I ~ SS_status,
  data = birds_insurance_hi_pa[birds_insurance_hi_pa$PA_I >= 1, ]
)
FSA::dunnTest(
  birds_insurance_hi_pa[birds_insurance_hi_pa$PA_I >= 1, ]$PA_I ~
    birds_insurance_hi_pa[birds_insurance_hi_pa$PA_I >= 1, ]$SS_status,
  method = "bonferroni"
)
...
sub_birds_insurance_pa <- birds_insurance_hi_pa[
  birds_insurance_hi_pa$PA_I_II >= ThtPA,
]
...
kruskal.test(
  PA_I_II ~ SS_status,
  data = birds_insurance_hi_pa[birds_insurance_hi_pa$PA_I_II >= 1, ]
)
FSA::dunnTest(
  birds_insurance_hi_pa[birds_insurance_hi_pa$PA_I_II >= 1, ]$PA_I_II ~
    birds_insurance_hi_pa[birds_insurance_hi_pa$PA_I_II >= 1, ]$SS_status,
  method = "bonferroni"
)
