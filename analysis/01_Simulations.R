...
traits <- cbind.data.frame(
  Species = paste0(rep("SP", 5000), 1:5000),
  T1 = rnorm(5000, sd = 10),
  T2 = rnorm(5000, sd = 10),
  T3 = rnorm(5000, sd = 10)
)
...
dist_mat = funrar::compute_dist_matrix(
  traits[, !colnames(traits) %in% "Species"],
  metric = "euclidean"
)
...
Betafunk <- function(insured, insurer, occ_list, traits) {
  ...
  test.pair_funk <- betapart::functional.beta.pair(
    x = df,
    traits = traits.sub,
...
fig_pc_insured_1 <- ggplot(pc_axes, aes(x = Axis1, y = Axis2)) +
  ...
  ylim(
    min(min(pc_axes$Axis1), min(pc_axes$Axis2)),
    max(max(pc_axes$Axis1), max(pc_axes$Axis2))
  ) +
  xlim(
    min(min(pc_axes$Axis1), min(pc_axes$Axis2)),
    max(max(pc_axes$Axis1), max(pc_axes$Axis2))
  ...
  pbmcapply::pbmclapply(
    ...
    function(i) {
      ...
      if ((length(spe_insured) > 10) & (length(spe_insurer) > 10)) {
        out <- do.call(
          rbind,
          lapply(Dthr_grad, function(Dthr) {
            do.call(
              rbind,
              lapply(D_insu_grad, function(D_insu) {
...
distinct_sim_90$diff_div <- distinct_sim_90$Div_insured -
  distinct_sim_90$Div_insurer

distinct_sim_90 <- distinct_sim_90[
  order(distinct_sim_90$diff_div, decreasing = F),
]
distinct_sim_90 <- distinct_sim_90[
  sample(1:nrow(distinct_sim_90), nrow(distinct_sim_90)),
]


b <- ggplot(
  distinct_sim_90,
  aes(x = funct.beta.jac, y = Inward_D, color = Div_insured)
) +
  ...
  geom_text(
    ...
    label = paste0(
      ...
      round(
        cor(
          distinct_sim_90$funct.beta.jac,
...
c <- ggplot(
  distinct_sim_90,
  aes(x = funct.beta.jac, y = Inward_C, color = Div_insured)
) +
  geom_point(alpha = 0.7, size = 0.9) +
  geom_density_2d(color = "black", alpha = 0.2, size = 0.3) +
  ylim(0, 100) +
...
d <- ggplot(
  distinct_sim_90,
  aes(x = Inward_D, y = Outward_D, color = Div_insured)
) +
  geom_point(alpha = 0.7, size = 0.9) +
  ylim(0, 100) +
  xlim(0, 100) +
...
ggsave(
  file = here::here("outputs", "FIG_3.tiff"),
  FIG_3,
  width = 15,
  height = 12,
  dpi = 300,
...
ggsave(
  file = here::here("outputs", "FIG_3_legend.tiff"),
  legend,
  width = 15,
  height = 15,
  dpi = 300,
...
b <- ggplot(
  distinct_sim_90,
  aes(x = funct.beta.jne, y = Inward_D, color = Div_insured)
) +
  geom_point(alpha = 0.7) +
  geom_density_2d(color = "black", alpha = 0.2, size = 0.3) +
  ylim(0, 100) +
  ...
  geom_text(
    ...
    label = paste0(
      ...
      round(
        cor(
          distinct_sim_90$funct.beta.jne,
          distinct_sim_90$Inward_D,
          use = "pairwise.complete.obs"
        ),
        2
...
c <- ggplot(
  distinct_sim_90,
  aes(x = funct.beta.jtu, y = Inward_D, color = Div_insured)
) +
  geom_point(alpha = 0.7) +
  geom_density_2d(color = "black", alpha = 0.2, size = 0.3) +
  ylim(0, 100) +
  ...
  geom_text(
    ...
    label = paste0(
      ...
      round(
        cor(
          distinct_sim_90$funct.beta.jtu,
          distinct_sim_90$Inward_D,
          use = "pairwise.complete.obs"
        ),
        2
...
e <- ggplot(
  distinct_sim_90,
  aes(x = funct.beta.jne, y = Inward_C, color = Div_insured)
) +
  geom_point(alpha = 0.7) +
  geom_density_2d(color = "black", alpha = 0.2, size = 0.3) +
  ylim(0, 100) +
...
f <- ggplot(
  distinct_sim_90,
  aes(x = funct.beta.jtu, y = Inward_C, color = Div_insured)
) +
  geom_point(alpha = 0.7) +
  geom_density_2d(color = "black", alpha = 0.2, size = 0.3) +
  ylim(0, 100) +
...
ggsave(
  file = here::here("outputs", "S1_Fig_1.tiff"),
  S1_Fig_1,
  width = 20,
  height = 12,
  dpi = 300,
