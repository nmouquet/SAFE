...
#'      - outputs/Fig_4d.tiff
#'
#'      Note : Fig_4a is produced with tmap (lines 782-800) and pasted into the final figure
#'             Fig 4c left is produced with tmap (lines 1004-1022) and pasted into the final figure
#'
...
# Create the map for all categories
subper = 1
...
#Create the map for only sources and sinks only Fig_4a
...
####ONE CELL WITHIN A BUFFER Fig 4c left----

focal_cell = "CAL1_848810"
id <- which(sub_divgrass_insurance$com %in% focal_cell)
reference_point <- sub_divgrass_insurance[id, ]
